...
function Feynman(heightA, widthA, container, reaction) {
	...
	var paper = Raphael(container, heightA, widthA);
	...
	{
		nstepx = paper.width / 5;
		nstepy = paper.height / 5;	
	}
	...
	function Line(vert, pidA, paper) {
		...
		this.DrawStraightLine = function() {
			...
			if (this.direction == -1)
				dir_angle = 180;
			//turn the arrow in the opposite direction
			...
			y2 = mean.GetY();
			size = 8;
			// change it to change dimension of the arrow!!!
			if (nstepy < 40) {
				nstepy = 40;
			}
			if (pid > 0) {
			...
			textA.hover(function() {
				this.attr("fill", ColorOfParticles[pid - 1]);
				MySize = this.attr('font-size');
				this.attr({
					"font-size" : MySize + 20
				});
			}, function() {
			...
			pathA.hover(function() {
				this.attr("stroke", ColorOfParticles[pid - 1]);
			}, function() {
			...
			textA.click(function() {
				...
				PanelDescription.click(function() {
					this.remove();
					TextDescription.remove();
				});
	...
	document.getElementById("Canvas").onclick = function(e) {
		var x = event.clientX + document.body.scrollLeft - document.getElementById(container).offsetLeft;
		var y = event.clientY + document.body.scrollTop - document.getElementById(container).offsetTop;
...
function svg_path_coil(x0, y0, len) {
	offset = 10;
	//length of the straight line at the beginning and end
...
function svg_path_sin(x0, y0, len) {
	...
	//var dx=(2*pi/N);
	var dx = 0.5;
	// augment dx to get page load faster
	var N = (len) / dx;
